...
const NewPostForm: React.FC<NewPostFormProps> = ({ onPost }) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // Reset state
    setPost({ restaurant: "", meal: "", rating: 0, photo: null });

    // Safely reset file input using the ref
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-4 mb-6">
      ...
      <div className="mb-4">
        <label className="block text-sm font-medium text-black">Upload a Photo</label>
        <input
          type="file"
          accept="image/*"
          name="photo"
